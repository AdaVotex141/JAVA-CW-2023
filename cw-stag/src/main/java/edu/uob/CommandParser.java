...
public class CommandParser {
    ...
    private String multipleTriggerParse(HashSet<String> triggerWords) {
        int count = 0;
        String trigger = "";
        GameAction previousGameAction = new GameAction();
            ...
            if (actionParser.actions.containsKey(triggerWord)) {
                ...
                for (GameAction gameAction : gameActionHashSet) {
                    if (gameAction.subjects.containsAll(entityCommand) && !entityCommand.isEmpty()) {
                        if(previousGameAction.narration == null){
                            previousGameAction = gameAction;
                        }else if(checkEqualGameAction(previousGameAction,gameAction)){
                            trigger = triggerWord;
                            count+=1;
                        }
                    }
    ...
    private Boolean checkEqualGameAction(GameAction action1, GameAction action2){
        boolean subjectsEqual = action1.subjects.equals(action2.subjects);
        boolean consumedEqual = action1.consumed.equals(action2.consumed);
        boolean producedEqual = action1.produced.equals(action2.produced);
        boolean narrationEqual = Objects.equals(action1.narration, action2.narration);
        return subjectsEqual && consumedEqual && producedEqual && narrationEqual;
    }
